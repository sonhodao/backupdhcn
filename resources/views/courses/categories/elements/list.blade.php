                        ...
                        $traverse = function ($categories) use (&$traverse, $currentCategory) {
                            foreach ($categories as $category) {
                                ...
                                if (request()->user()->can('categories.update')) {
                                    echo '<span class="prevent-expand ml-1 btn btn-warning btn-sm" onclick="$.pjax({url: \''.route('course-categories.index', ['id' => $category->id]).'\', container: \'#pjax-container\'});">'.__('Edit').'</span>';
                                }

                                if (request()->user()->can('categories.destroy')) {
                                    echo '<span class="prevent-expand ml-1 btn btn-danger btn-sm" onclick="deleteResource(\''.route('course-categories.destroy', ['course_category' => $category->id]).'\', \''.route('course-categories.index').'\')">'.__('Delete').'</span>';
                                }
