...
@section('content')
    <div class="row">
        ...
        @php
            ...
            if (!request()->user()->can('course-categories.store')) {
                $showForm = false;
            }

            if (!empty($currentCategory)) {
                $showForm = request()->user()->can('course-categories.update');
            }
        @endphp

        @if ($showForm)
            <div class="col-md">
                <form method="post" class="card card-primary card-outline card-outline-tabs" action="{{ !empty($currentCategory) ? route('course-categories.update', ['course_category' => $currentCategory->id]) : route('course-categories.store') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="card-body">
                        <div class="tab-content" id="category-tabs-content">
                            <div class="tab-pane fade show active" id="category-tabs-info" role="tabpanel" aria-labelledby="category-tabs-info-tab">
                                @include('courses.categories.elements.info')
                            </div>
                        </div>
                    </div>
        ...
        @include('courses.categories.elements.list')
    </div>
@endsection
